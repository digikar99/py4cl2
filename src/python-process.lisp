...
(defun pystart (&optional (command (config-var 'pycmd)))
 ...
 By default this is is set to (CONFIG-VAR 'PYCMD)"
  ...
  (let ((python (make-python-process-info)))
    (loop
       ...
       (iter outer
         ...
         (handler-case
             (loop :for char := (read-char py-out)
                   :while (not (python-thread-end-signal pyinfo))
                   :do (write-char char))
           ;; Python process can quit even with a user sent "quit(0)"
           ;; It does not depend on python-thread-end-signal
           (simple-error (condition)
             (when (uiop:process-alive-p pyinfo)
               (cerror "~S~%  ~A~%occured while inside python-output-thread ~A"
                       condition condition (bt:current-thread))))
           (stream-error (condition)
             (when (and (uiop:process-alive-p pyinfo)
